...
resource "aws_cloudwatch_event_target" "backup_restore" {
  ...
  arn       = "arn:aws:lambda:${data.aws_region.current.name}:${local.account_id}:function:nutrien-${var.environment}-backup-restore"
}
...
resource "aws_lambda_permission" "backup_restore" {
  ...
  function_name = "nutrien-${var.environment}-${var.lambda_function_config["backup-restore"].function_suffix}"
